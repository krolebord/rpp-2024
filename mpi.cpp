...
int main(int nargs, char* args[])
{
    ...
    std::cout << "proc_rank: " << proc_rank << std::endl;

    int current_element = 0;
    int numbers_per_part = ceil((double)counts / (double)(numprocs));

    if (proc_rank == 0) {
        for (int destination_process = 1; destination_process < numprocs; destination_process++) {
            int index_offset = destination_process * numbers_per_part;
    ...
    for (int i = 0; i < counts; i++) {
        ...
        for (int j = 0; j < numbers_per_part; j++) {
            auto curr = proc_rank == 0 ? a[j] : bufNumbers[j];
            if (curr == leftover) {
                f = true;
            }
        }
